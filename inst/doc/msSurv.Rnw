...
calculates and returns the marginal state occupation probabilities
for a general, possibly
non-Markov, multistate system, and provides features not
currently found in  other \proglang{R} packages.
Additionally, state entry and exit time distributions are calculated
and returned for states where it is appropriate.
The function also calculates and returns the
marginal integrated transition hazards and the hazard rate functions, and for
a Markov model, the transition probability matrix between any two times.
...
Usually, one assumes that censoring is independent of the multistate
system.  However, there are times when censoring and  hazards
...
independent given the covariate process $Z$ \citep{DS2001, DS2002}.
...
Nelson-Aalen estimator of censoring \citep{DS2002}.  Currently, the
\pkg{msSurv} package only implements this type of dependent
censoring.
...
the state entry and exit time distribution functions.
We assume that the only path between the states before and after the
state of interest (e.g., state $j$) is through state $j$, and that the
system is acyclic with respect to state $j$ (i.e., it is not possible
to return to state $j$ after leaving it).
% We assume an acyclic system.
Suppose $X_j$ denotes the possibly unobserved indicator of an individual ever
...
the \rfarg{var = TRUE} argument in the call to the \rf{EntryExit}
...
Figure~\ref{fig:DepExSOP}.

<<DepExSOP, fig=TRUE, eval=TRUE, include=FALSE>>=
if(exists("DepEx")) plot(DepEx)
@

\begin{figure}[ht]
\begin{center}
  \includegraphics[height=5in,width=5in]{"msSurv-DepExSOP"}
...
The default plot produced for transition probabilities plots estimates
and confidence intervals for all possible transitions in the system.

<<DepExTransPlot, fig=TRUE, eval=TRUE, include=FALSE>>=
if(exists("DepEx"))
    plot(DepEx, plot.type = "transprob")
@

The resulting plot is given in Figure~\ref{fig:DepExTransPlot}.
...
<<DepExTransPlot1, fig=TRUE, eval=FALSE, include=FALSE>>=
if(exists("DepEx"))
...
\begin{figure}[ht]
\begin{center}
  \includegraphics[height=5in, width=5in]{"msSurv-DepExTransPlot"}
  ...
  \label{fig:DepExTransPlot}
\end{center}
\end{figure}

For state entry / exit distributions, in the current example the
conditions given for ensuring proper calculation of these distributions  (see
Section~\ref{subsec:FsGs}) are only satisfied for states 1 and 5.  To
calculate state entry / exit distributions for the transient states,
say states 2 and 3, we need to expand the model to differentiate
between paths to states 4 and 5 that go through state 2 and paths that
go through state 3.  The expanded system is plotted in
Figure~\ref{fig:EntryExitDepExStateModel}, where states 4 and 5 have
been relabeled as 4-2 and 5-2 for paths that go through state 2 and
4-3 and 5-3 for paths that go through state 3. The data needs to be
modified correspondingly to incorporate these additional states.  Note
that individuals who start in state 4 are dropped from this model, as
these individuals have no impact on the entry / exit distributions
from states 2 and 3.  The updated tree and data are in objects
\ro{deptree2} and \ro{data.sdc2}, respectively.


%% NEW ADDITIONS HERE
<<EntryExitDepExData, eval = FALSE, echo = FALSE, results = hide>>=
...
<<EntryExitDepExStateModel, fig = TRUE, echo = FALSE, include = FALSE, eval = FALSE>>=
...
\begin{figure}
  \begin{center}
    \includegraphics[height=3in,width=3in]{msSurv-EntryExitDepExStateModel.pdf}
    \caption{Expanded model discussed in Section~\ref{subsec:DepEx},
      which differentiates between paths to states 4 and 5 that go
      through state 2 ($2\rightarrow$ 4-2 and $2 \rightarrow$ 5-2) and
      state 3 ($3\rightarrow$ 4-3 and $2 \rightarrow$ 5-3).}
    \label{fig:EntryExitDepExStateModel}
  \end{center}
\end{figure}

%% NOTE - Change to eval = FALSE after running once to get figures
The updated model is fitted below, and the normalized state entry
distribution plots for states 2 and 3 over the time range of 0 to 100
days are given in Figure~\ref{fig:NormEntryDepExPlot}. The
corresponding normalized exit distributions are plotted in
Figure~\ref{fig:NormExitDepExPlot}.  By default (i.e., with the
\rfarg{states="ALL"} argument) all states where calculation of entry / exit
distributions is valid are plotted.


<<EntryExitDepExFit, echo = TRUE, eval = FALSE>>=
if(exists("data.sdc2"))
    DepEx2 <- msSurv(data.sdc2, deptree2, cens.type = "dep", bs = TRUE)
@


<<NormEntryDepExPlot, fig=TRUE, eval=FALSE, include=FALSE>>=
...
\begin{figure}[ht]
\begin{center}
  \includegraphics[height=5in, width=5in]{"msSurv-NormEntryDepExPlot"}
  \caption{Plot of normalized state entry time distribution estimates
    and corresponding confidence intervals for states 2 and 3 in the
    system initially illustrated in Figure~\ref{fig:depcensimage} and expanded
    in Figure~\ref{fig:EntryExitDepExStateModel}.}
  \label{fig:NormEntryDepExPlot}
\end{center}
\end{figure}

<<NormExitDepExPlot, fig=TRUE, eval=FALSE, include=FALSE>>=
...
\begin{figure}[ht]
\begin{center}
  \includegraphics[height=5in, width=5in]{"msSurv-NormExitDepExPlot"}
  \caption{Plot of normalized state exit time distribution estimates
    and corresponding confidence intervals for states 2 and 3 in the
    system initially illustrated in Figure~\ref{fig:depcensimage} and expanded
    in Figure~\ref{fig:EntryExitDepExStateModel}.}
  \label{fig:NormExitDepExPlot}
\end{center}
\end{figure}
...
state 2 (acute GVHD) or state 3 (normal platelets) by day 100
(c.f.~Figures~\ref{fig:DepExSOP} and \ref{fig:NormEntryDepExPlot}).    By
70 days post-transplant, roughly 75\% of the patients have had their
platelet levels return to normal before development of acute GVHD (Figure~\ref{fig:DepExSOP}).
The normalized entry time distribution for state 3 reaches
approximately 1.0 by 100 days,
...
Figure~\ref{fig:NormExitDepExPlot}),
with a small percentage subsequently developing acute GHVD
($3\rightarrow 4$ transition in Figure~\ref{fig:DepExTransPlot}) and 60\%
eventually going on to develop chronic GVHD ($3\rightarrow 5$
transition in Figure~\ref{fig:DepExTransPlot}). A much smaller
percentage of patients first develop acute GVHD ($1\rightarrow 2$
transition in Figure~\ref{fig:DepExTransPlot}), with about 40\% of
these having their platelet levels returning to normal by 70-80 days
(peak of $2\rightarrow 4$ transition in Figure~\ref{fig:DepExTransPlot}).
All of the patients who develop acute GVHD first eventually go on to
develop chronic GVHD ($2\rightarrow 5$ transition in
Figure~\ref{fig:DepExTransPlot}).


\subsection{An illustration of state-dependent censoring using simulated
  data}
\label{subsec:stateDepIllDeath}

In the previous example based on real data the dependency of the
censoring distribution on the state in the system was not too heavy,
and the difference between estimates assuming state-dependent and
state-independent censoring was relatively minor (data not shown).
In this example we illustrate the magnitude of the difference between
the two estimates that can
occur using simulated data in the same spirit as \citet{DS2002}.
% the difference between the
% Aalen-Johansen and Datta-Satten (assuming state-dependent censoring) estimates
Data were simulated from a three state irreversible
illness-death model, where all individuals begin in state 1 (well) at time 0.
Individuals in state 1 may subsequently transition to state 2
(illness) or state 3 (death).  Individuals
with illness remain in state 2 until death (entry
into state 3). For each individual, a vector $W = (W_l, W_2,
W_3)$ was generated from a trivariate normal distribution with
mean vector (1.81, 1.7, 1.7) and variance-covariance matrix $0.393 \times
(0.01\textnormal{\textbf{I}} + 0.91\mathbf{1}\mathbf{1}^T)$, where
\textbf{I} is the identity matrix and $\mathbf{1}$ is
a $3\times 1$ vector of ones. The values
of $W$ are (log) latent transition times with the following possible
transition paths:
\begin{description}
  \item[$W_1 < W_2$:] Death occurs before illness at time
    $\exp(W_1)$
  \item[$W_2 < W_1$:] Illness occurs at time $\exp(W_2)$ and
    death at time $\exp(W_2) + \exp(W_3)$
\end{description}
The hazard for censoring was state dependent with censoring times
following a Weibull distribution. In state 1, the censoring hazard was
$t^{1/4}/60$, while in state 2 the censoring hazard was $t/50$.
For each observation, a state 1 censoring time $C_1$ was generated and
if $C_1 < \textnormal{min}\{\exp(W_l), \exp(W_2)\}$ the observation was
censored in state 1.  If $\exp(W_2) < C_1$ and if state 2 was
entered, then a state 2 censoring time was generated from the
Weibull distribution conditional on $C_2 > \exp(W_2)$.  Simulations
were based on a single data set of 1000 subjects.
Figure~\ref{fig:stateDepIllDeath} illustrates the state 2 occupation
probability estimates based on
the complete (non-censored) data, the Aalen-Johansen estimates, and
the Datta-Satten estimates assuming state-dependent censoring.  The
Datta-Satten estimates closely follow the empirical estimates based on
non-censored data, while the Aalen-Johansen estimates deviate
appreciably.


<<StateDependentCensoringIllnessDeath, eval=FALSE, echo=FALSE, results=hide>>=
...
<<StateDependentCensoringIllnessDeathPlot, fig=TRUE, eval=FALSE, include=FALSE, echo=FALSE>>=
####################################################################
...
\begin{figure}[ht]
\begin{center}
  \includegraphics[height=5in,width=5in]{"msSurv-StateDependentCensoringIllnessDeathPlot"}
  \caption{Plot of state 2 occupation probabilities for the
    three-state irreversible illness-death model in
    Section~\ref{subsec:stateDepIllDeath}. The Datta-Satten (DS)
    estimator closely follows the empirical estimator based on
    complete (non-censored) data, while the Aalen-Johansen (AJ)
    estimator deviates noticeably. }
  \label{fig:stateDepIllDeath}
\end{center}
\end{figure}
